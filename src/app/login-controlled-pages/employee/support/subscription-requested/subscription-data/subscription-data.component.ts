...
export class SubscriptionDataComponent implements OnInit {
  ...
  updateSubscriptionProperty(key: string, event: any, data_type: string, deselected: boolean = false, isAllOPeration: boolean = false) {
    CommonUtilityFunctions.updateRecordProperty(key, event, data_type, this.updatedSubscriptionRecord, this.subscriptionRecord.property, deselected, isAllOPeration);
  }

  updateSubscriptionRecord() {
    const data = CommonUtilityFunctions.encodeFormDataToUpdatedJSONWithParentSerialId(this.updatedSubscriptionRecord, this.subscriptionRecord.getProperty('tentativeSubscriptionId'));
